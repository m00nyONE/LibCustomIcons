...
def get_folders() -> List[Folder]:
    """Return folder names derived from .lua filenames in cwd."""
    return [item[:-4] for item in os.listdir() if item.endswith(".lua")]
        ...
        with open(lua_path, "r", encoding="utf-8") as fil:
            for raw_line in fil:
                ...
                for (folder, imageName), (w, h, left, right, top, bottom, total_w, total_h) in coord_map.items():
                    ...
                    replaced_line = (
                        f'{prefix} {{"LibCustomIcons/icons/compiled/merged_{w}x{h}.dds", '
...
def write_output(lines_out: List[str], output_path: str = 'compiled/static.lua') -> None:
